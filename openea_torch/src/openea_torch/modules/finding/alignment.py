...
from openea_torch.modules.utils.util import task_divide, merge_dic
...
def retrieve_topk_alignment(kg1_source_ents, kg1_embeddings, kg2_candidates, kg2_embeddings, device, k=1,
    ...
    def triple_writer(triples, output_path, separator="\t", linebreak="\n"):
        with open(output_path, 'w', encoding='utf8') as file:
            for s, p, o in triples:
                file.write(f"{s}{separator}{p}{separator}{o}{linebreak}")
        print(output_path, "saved")

    kg1_embeddings = torch.tensor(kg1_embeddings, device=device)
    kg2_embeddings = torch.tensor(kg2_embeddings, device=device)
    kg1_source_ents = torch.tensor(kg1_source_ents, device=device)
    kg2_candidates = torch.tensor(kg2_candidates, device=device)

    embeds1 = kg1_embeddings[kg1_source_ents]
    embeds2 = kg2_embeddings[kg2_candidates]
    sim_mat = sim(embeds1, embeds2, metric=metric, normalize=normalize, csls_k=csls_k)
    topk_neighbors = search_nearest_k(sim_mat, k)
    topk_neighbors_w_sim = [(kg1_source_ents[i].item(), kg2_candidates[j].item(), sim_mat[i, j].item())
