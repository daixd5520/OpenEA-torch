...
def read_links(file_path):
    ...
    with open(file_path, 'r', encoding='utf8') as file:
        for line in file.readlines():
            ...
            print(params)
            print(f"len:{len(params) }")
            assert len(params) == 2, "Each line should contain exactly two elements separated by a tab."
            e1 = params[0].strip()  # 这里也可以去掉，因为已经strip过了
            e2 = params[1].strip()  # 同上
            refs.append(e1)
            reft.append(e2)
            links.append((e1, e2))
    assert len(refs) == len(reft), "The number of elements in 'refs' and 'reft' should be the same."
    return links
